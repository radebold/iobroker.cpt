...
class CptAdapter extends utils.Adapter {
    ...
    async onMessage(obj) {
        try {
            ...
            // Always log received messages in beta to debug sendTo buttons
            this.log.info(`[onMessage] from=${obj.from} cmd=${obj.command} msg=${JSON.stringify(obj.message)}`);

            // Channel test from Communication tab
            if (obj.command === 'testChannel') {
                const text = obj.message?.text || 'TEST: CPT Nachricht';
                const onlyInstance = obj.message?.instance || obj.message?.adapter || obj.message?.to || '';
                await this.sendMessageToChannels(text, { onlyInstance });
                if (obj.callback) this.sendTo(obj.from, obj.command, { ok: true }, obj.callback);
                return;
            }

            // Station test button from Stations table
            if (obj.command === 'testStation') {
                const stationName = obj.message?.stationName || obj.message?.name || '';
                let stationPrefix = obj.message?.prefix || '';

                this.log.info(`[testStation] clicked stationName=${stationName} prefix=${stationPrefix}`);

                // If prefix not provided: find by stations.*.*.name
                if (!stationPrefix && stationName) {
                    const all = await this.getStatesAsync(this.namespace + '.stations.*.*.name');
                    const target = stationName.toLowerCase().trim();
                    for (const [id, st] of Object.entries(all || {})) {
                        if (st && String(st.val).toLowerCase().trim() === target) {
                            stationPrefix = id.replace(/\.name$/, '').replace(this.namespace + '.', '');
                            break;
                        }
                    }
                }
                ...
                if (!stationPrefix) {
                    this.log.warn(`[testStation] Station not found: ${stationName}`);
                    if (obj.callback) this.sendTo(obj.from, obj.command, { ok: false, error: 'station_not_found' }, obj.callback);
                    return;
                }
