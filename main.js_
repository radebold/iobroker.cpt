...
`;
        ...
        for (const p of prefixes) {
            ...
            const placeTitle = city ? `${esc(city)} · ${esc(name)}` : esc(name);
            out += `
      <div style="border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:8px 10px;background:rgba(255,255,255,.03);">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;">
          <div style="min-width:0;flex:1 1 auto;">
            <div style="font-size:15px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${placeTitle}</div>
          </div>
          <div style="flex:0 0 auto;text-align:right;display:flex;flex-direction:column;align-items:flex-end;gap:6px;">${portsSummary}${badge(st || '—', kindFromStatus(st))}</div>
        </div>
        <div style="margin-top:8px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">${distText}${ageBadge}${portText}</div>
      </div>
`;
        }
